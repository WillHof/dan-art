            ...
            <div className="dropdown">
                ...
                <div className="dropdown-menu" aria-labelledby="pDrop">
                    {this.props.allCategories.map((currC, index) =>
                        (
                            <span key={index} value={currC} className={"quicksand" + this.props.category === currC ? "dropdown-item" : "dropdown-item tgray4"} onClick={(e) => this.props.onClick(e)}>{this.fixName(currC)}</span>
                        ))}
                </div>
            </div>
        )
